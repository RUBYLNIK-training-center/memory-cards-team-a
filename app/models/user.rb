...
# Database Authenticatable: предоставляет возможность входа в систему на основе зашифрованного и хранимого в базе данных
# пароля. Вход может быть выполнен посредством отправки POST-запроса или с помощью HTTP Basic Authentication.
...
# Validatable: предоставляет инструменты валидации e-mail и пароля. Модуль может быть легко настроен, вы можете
# определить собственные валидаторы.
# Lockable: блокирует аккаунт после указанного в настройках количество неудачных попыток авторизации. Аккаунт может
# быть разблокирован посредством email или через определенный период времени.
